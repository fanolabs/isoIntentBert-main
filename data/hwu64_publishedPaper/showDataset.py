...
def display_data(data):
    ...
    # check duplicate and empty utterance
    uttList = []
    for domain in data:
        for d in data[domain]:
            lab = d[1][0]
            uttList.append(d[0])
    uttSet = set(uttList)
    print("[INFO] After removing duplicate utterances: %d -> %d"%(len(uttList), len(uttSet)))
    print("[INFO] Empty utterances: %d"%(len([utt for utt in uttList if len(utt) == 0])))
